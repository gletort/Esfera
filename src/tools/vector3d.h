...
class Vector3d
{
	...
	public:
		...
		inline double distanceToXaxis() const { return sqrt( data[1]*data[1] + data[2]*data[2] ); }
		/** \brief Get distance to y-axis */
		inline double distanceToYaxis() const { return sqrt( data[0]*data[0] + data[2]*data[2] ); }
		/** \brief Get distance to z-axis */
		inline double distanceToZaxis() const { return sqrt( data[0]*data[0] + data[1]*data[1] ); }

		/** \brief Assign random values to current vector, each component follows N(0,1) */
		void randomNormal();

		/** \brief Assign random values to current vector, with norm 1 */
		void randomAndNormed();
