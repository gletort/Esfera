...
class ReadXML
{
	private:
		...
		XMLElement* pInit;
		/** Contains parameters of space, nb 2 */
		XMLElement* pSpace;
		/** Contains parameters of dna, nb 3 */
		XMLElement* pDNA;
		...
		/** number of categories apart of spheres props */
		int nbCat;
	...
	public:
		...
		void loadXMLFile( std::string cat0, std::string cat1, std::string cat2, std::string cat3, std::string ncat) throw();

		/** \brief Returns number of sphere types defined */
		inline int nbProps() { return (int) props.size(); };
...
XMLElement* getFirstChild( std::string where, std::string name, std::string sub = "", std::string subsub = "" );
	...
	/** \brief Retrieve double value of parameter name and replace value if found
