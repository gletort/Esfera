...
void SphereSet::switchNow( double contact )
{
	std::vector<double> dists; // list of distances to DNA
		for ( auto sph : spheres )
		{
			double d = (sph.second)->distanceToDNA();
			if ( d > -1000 ) dists.push_back(d);
		}
		// sort and get first contact quantile
		std::sort(dists.begin(), dists.end());
		int index = dists.size()*contact;
		double dlim = dists[index];
		// switch to contact all spheres closer than dlim
		for ( auto sph : spheres )
		{
			(sph.second)->contactNow(dlim);
		}
