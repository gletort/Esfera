...
void Sphere::addBrownianD(double dt, double activity)
{
	...
	else  // diffusion of speckle in contact with dna
	{
		...
		// small attraction toward dna surface
		//speed +=   (prop->dna)->surfaceAttraction();
		// Follow DNA movement
	...
	else
	{
		...
		if ( random->uniform() <= (prop->dna_mode*dt) )
		{
			if ( ! prop->dna_src )
			{
				// diffuse only if not block by DNA
				// Blockage by DNA: random proba according to distance to nucleole
			if (! (prop->dna)->contactRate1(pos, radius, dt) )
			{
				speed += sqrt(2*(cur_diff_0 + cur_diff_alpha * activity)/dt) * ran;
			}	
			}
...
void Sphere::oneStep( double dt, Space* space, double activity, bool init )
{
	Vector3d disSurf;
	double rad = radius;
	// add thickness if nucleole
	//if ( prop->dna_src == 1 )
	//	rad += (prop->dna)->getThickness();
